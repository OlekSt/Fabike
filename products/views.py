...
def products(request):
    ...
    if request.GET:
        if 'q' in request.GET:
            ...
            queries = Q(name__icontains=query) | Q(title__icontains=query) |\
                Q(frame__icontains=query) | Q(fork__icontains=query) |\
                Q(wheels__icontains=query) | Q(tyres__icontains=query) |\
                Q(crankset__icontains=query) |\
                Q(shift_levers__icontains=query) |\
                Q(casette_or_sprocket__icontains=query) |\
                Q(chain_or_belt__icontains=query) |\
                Q(brakes__icontains=query) |\
                Q(bottom_bracket__icontains=query) |\
                Q(dropouts__icontains=query)
...
def edit_product(request, product_id):
    ...
    if request.method == 'POST':
        ...
        messages.info(request, f'You are editing {product.frame}\
                                                 {product.name}')
